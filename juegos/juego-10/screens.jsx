// screens.jsx — JUEGO-10 · "Ecuador megadiverso" (Estudios Sociales).
// Juego MULTI-TEMA: el Home muestra N BOTONES = N temas del libro (patrón EDINUN).
// El niño elige un tema (chip) + su nombre → CharacterScreen (Yaku preseleccionado) →
...
// ─────────────────────────────────────────────────────────────
// Fondo cósmico + glifos flotantes — megadiversidad del Ecuador: flora y fauna de las
// cuatro regiones naturales (🐢 🦜 🐸 🦎 🌵 🌸 🌿 🌴 🍌 🐆 🦅 🐬) y relieve/paisaje
// (🌋 🏔️ 🌊). Decorativos, baja opacidad: cosmic 15 · chalkboard 10.
// ─────────────────────────────────────────────────────────────
...
function HomeScreen({ app, setApp, go }) {
  ...
  const initial = app.level && LEVELS_CFG.some((l) => l.id === app.level && l.enabled) ? app.level : "recursos";
...
// ─────────────────────────────────────────────────────────────
// 2. SELECCIÓN DE PERSONAJE — Yaku preseleccionado (ciclo del elenco por ordinal del
// slug: 10 mod 4 = 2 → yaku; ver memory/orden-personajes.md).
// Mapea el tema elegido en Home a currentCategory/currentCatLabel que usa GameScreen.
// ─────────────────────────────────────────────────────────────
function CharacterScreen({ app, setApp, go }) {
  // ⚠ `app.character || "yaku"` NO sirve: `app.jsx` (shell, no se toca) inicializa
  // character:"domi", que es truthy, así que el preseleccionado quedaba en Domi. Se
  // descarta explícitamente ese valor por defecto, igual que hace juego-8 con Andi.
  const [sel, setSel] = useState(app.character && app.character !== "domi" ? app.character : "yaku");
