...
function SEShell({ app, setApp, go, ronda, startedAt, enunciado, enunciadoG, bocadillo, actions, onRestart, rootRef, overlay, children }) {
  const char = CHARACTERS.find((c) => c.id === app.character) || CHARACTERS[0];
  const [confirmingExit, setConfirmingExit] = useStateG(false);
  const [confirmingRestart, setConfirmingRestart] = useStateG(false);
...
function AdivinaDatoRound({ app, setApp, go, ronda, startedAt, onDone, onRestart }) {
  ...
  function confirmar() {
    ...
    setShown(true);
    // Avanza SOLA tras el reveal (invariante §6: "en quiz, avanzar automático").
    // No lleva botón SIGUIENTE: eso no existe en el estándar.
    setTimeout(() => onDone(logRef.current.slice()), 3200);
  }
...
function MitosDatosRound({ app, setApp, go, ronda, startedAt, onDone, onRestart }) {
  ...
  const [left, setLeft] = useStateG(SE_MITO_SEGS);
  const logRef = useRefG([]);
  const firstDone = useRefG(false);
  const m = mitos[i];
  ...
  // Cuenta atrás: sólo corre mientras no haya respondido.
  useEffectG(() => {
    if (answered !== null) return undefined;
    setLeft(SE_MITO_SEGS);
    const t0 = Date.now();
    const id = setInterval(() => {
      const rest = SE_MITO_SEGS - Math.floor((Date.now() - t0) / 1000);
      if (rest <= 0) { clearInterval(id); setLeft(0); responder(null); }
      else setLeft(rest);
    }, 200);
    return () => clearInterval(id);
  }, [i, answered]);

  // Tras responder, avanza solo (más tiempo si falló: hay que leer el dato real).
  useEffectG(() => {
    if (answered === null) return undefined;
    const last = logRef.current[logRef.current.length - 1];
    const t = setTimeout(siguiente, last && last.ok ? 1300 : 2600);
    return () => clearTimeout(t);
  }, [answered]);

  const last = logRef.current[logRef.current.length - 1];
  const ok = !!(answered !== null && last && last.ok);
  const urgente = left <= 3 && answered === null;
    ...
    >
      ...
      <div style={{ width: "100%", display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ flex: 1, height: 8, borderRadius: 999, background: "rgba(0,0,0,0.4)", overflow: "hidden", border: "1px solid rgba(255,255,255,0.15)" }}>
          <div style={{ height: "100%", width: `${(left / SE_MITO_SEGS) * 100}%`, background: urgente ? "linear-gradient(90deg,#ff9b9b,#e74c3c)" : "linear-gradient(90deg,#7ec8ff,#2a7fd7)", transition: "width 0.2s linear" }} />
        </div>
...
function CompletaTitularRound({ app, setApp, go, ronda, startedAt, onDone, onRestart }) {
  ...
  function verificar() {
    ...
    setVerdict(vd);
    setTimeout(() => onDone(entries), 3200);   // deja leer el reveal
  }
...
function Tema3Controller({ app, setApp, go }) {
  ...
  function finish(entries) {
    ...
    // Completar el tema cuenta como éxito aunque haya errores (invariante EDINUN):
    // el cartel celebra si sacó al menos la mitad; si no, anima sin quitar lo ganado.
    const bien = aciertos * 2 >= log.length;
    setBanner(bien ? "ok" : "err");
    setBannerMsg(bien ? `+${aciertos} ⭐` : ANIMOS[Math.floor(Math.random() * ANIMOS.length)]);
    setTimeout(() => {
      setBanner(null); setBannerMsg("");
      setApp((s) => ({
        ...s,
      ...
      {banner && (
        <PortalToBody>
          <div style={{ position: "fixed", inset: 0, zIndex: 1000, pointerEvents: "none", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, background: "rgba(0,0,0,0.55)", backdropFilter: "blur(3px)", animation: "ed-pop-in 0.3s" }}>
            <div style={{ fontFamily: "'Fredoka','Baloo 2',system-ui,sans-serif", fontWeight: 700, fontSize: "clamp(56px, 11vmin, 120px)", color: banner === "ok" ? "#2ecc8f" : "#ff6b6b", textShadow: "0 4px 0 rgba(0,0,0,0.45), 0 0 60px currentColor", textAlign: "center" }}>
              {banner === "ok" ? "¡EXCELENTE!" : "¡UPS!"}
            </div>
            {bannerMsg && (
              <div style={{ fontFamily: "'Fredoka','Baloo 2',system-ui,sans-serif", fontWeight: 700, fontSize: "clamp(18px, 2.6vmin, 30px)", color: banner === "ok" ? "#fce9a8" : "#fff", background: "rgba(0,0,0,0.55)", padding: "8px 26px", borderRadius: 999, textShadow: "0 2px 6px rgba(0,0,0,0.6)", textAlign: "center" }}>
                {banner === "err" ? `${bannerMsg} — ${char.name}` : bannerMsg}
              </div>
            )}
          </div>
