...
function L5Foto({ slug, emoji, size = 140, prefix = "l5" }) {
  const [tryIdx, setTryIdx] = useStateG(0);
  ...
  return <img src={`assets/${prefix}-${slug}.${L5_IMG_EXTS[tryIdx]}`} alt="" draggable="false" onError={() => setTryIdx((i) => i + 1)} style={{ width: size, height: size, objectFit: "cover", borderRadius: 12, display: "block", boxShadow: "0 6px 16px rgba(0,0,0,0.3)" }} />;
}
...
function PR1Provincia({ onSolve }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", height: "100%", width: "100%", paddingTop: 24 }}>
      {/* paddingTop 24 (no 8): con 8 el enunciado caía en y=68 y los puntos de RONDA
          terminan en y=67 — se leían pegados. Con 24 respira y las opciones siguen con
          ~30 px de colchón hasta el borde de la zona de juego. */}
...
// Las 6 provincias de la Amazonía y su capital (textual del libro). Emoji de respaldo por
// provincia (símbolo/fauna que el libro nombra) para cuando aún no hay foto. `AmazoniaGame`
// usa un ARREGLO de rondas (L5T2_ROUNDS) para poder crecer de 1 a 3 sin refactor.
const L5T2_PROVINCIAS = [
  { id: "sucumbios", t: "Sucumbíos", cap: "Nueva Loja", e: "🐬" },      // delfín rosado
...
// R1 — elige 1 provincia (evitando recientes) y arma 4 capitales: la correcta + 3 distractoras.
function l5t2R1Build() {
  const recent = new Set(l3t2Recent(L5T2_R1_KEY)), all = L5T2_PROVINCIAS.map((_, i) => i);
  const idx = l3Shuffle(all.filter((i) => !recent.has(i))).concat(l3Shuffle(all.filter((i) => recent.has(i))))[0];
  l3t2Push(L5T2_R1_KEY, idx, 4);
  const prov = L5T2_PROVINCIAS[idx];
  const others = l3Shuffle(L5T2_PROVINCIAS.filter((_, i) => i !== idx)).slice(0, 3);
  return { prov, opts: l3Shuffle([prov].concat(others)) };
}

// ── R1: ¿Cuál es la capital de {provincia}? — TOCAR 1 de 4 ──
function PR1Capital({ onSolve }) {
  const [b] = useStateG(() => l5t2R1Build());
  const [picked, setPicked] = useStateG(null);
  const answered = picked !== null;
  function tap(p) {
    if (answered) return;
    setPicked(p.id);
    onSolve(p.id === b.prov.id, { emoji: "🏛️", a: `¿Cuál es la capital de ${b.prov.t}?`, userAnswer: p.cap, correctAnswer: b.prov.cap });
  }
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", height: "100%", width: "100%", paddingTop: 8 }}>
      <div style={{ pointerEvents: "none", textAlign: "center", textShadow: "0 2px 8px rgba(0,0,0,0.6)" }}>
        <span style={{ fontFamily: "var(--ed-font-display)", fontWeight: 700, fontSize: 22, color: "rgba(255,255,255,0.9)" }}>¿Cuál es la capital de…</span>
      </div>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 18, minHeight: 0, width: "100%" }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, background: "linear-gradient(180deg, rgba(255,255,255,0.96), rgba(240,235,225,0.92))", border: "3px solid #f2c260", borderRadius: 20, padding: "16px 28px", boxShadow: "0 12px 28px rgba(0,0,0,0.4)", maxWidth: 460 }}>
          <L5Foto slug={b.prov.id} emoji={b.prov.e} size={168} prefix="l5t2" />
          <span style={{ fontFamily: "var(--ed-font-display)", fontWeight: 800, fontSize: 24, color: "#3a2608", textAlign: "center", lineHeight: 1.1 }}>{b.prov.t}</span>
        </div>
        <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap", maxWidth: 470 }}>
          {b.opts.map((p) => {
            const isCorrect = p.id === b.prov.id;
            let border = "#f2c260", bg = "linear-gradient(180deg, #fff8e6, #f7e3a8)", col = "#3a2608";
            if (answered) {
              if (isCorrect) { border = "#2ecc8f"; bg = "linear-gradient(180deg, rgba(72,224,154,0.95), rgba(26,143,95,0.92))"; col = "#06381f"; }
              else if (p.id === picked) { border = "#ff6b6b"; bg = "linear-gradient(180deg, rgba(255,139,139,0.92), rgba(178,47,47,0.9))"; col = "#fff"; }
            ...
            return (
              <button key={p.id} onClick={() => tap(p)} disabled={answered}
                style={{ position: "relative", minWidth: 106, height: 60, borderRadius: 16, border: `3px solid ${border}`, background: bg, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 14px", cursor: answered ? "default" : "pointer", boxShadow: "inset 0 1px 0 rgba(255,255,255,0.7), 0 6px 14px rgba(0,0,0,0.3)" }}>
                {answered && (isCorrect || p.id === picked) && (
                  <span style={{ position: "absolute", top: 4, right: 6, fontSize: 17, fontWeight: 900, lineHeight: 1, color: isCorrect ? "#06381f" : "#fff" }}>{isCorrect ? "✓" : "✗"}</span>
                )}
                <span style={{ fontFamily: "var(--ed-font-display)", fontWeight: 800, fontSize: 16, color: col, textAlign: "center", lineHeight: 1.05 }}>{p.cap}</span>
              </button>
            );
          })}
        </div>
...
// ── R3: ARMA LA PALABRA — ordena las letras para escribir la capital (con dificultad) ──
// Dificultad: la bandeja trae las letras de la capital MÁS 2 letras señuelo (que no van), todo
// barajado; el niño coloca las letras EN ORDEN en las casillas y ¡VERIFICA!. Solo capitales de
// una palabra (Sucumbíos/Nueva Loja queda para R1). Anti-repetición por capital.
const L5T2_R3_KEY = "edinun_juego6_l5t2_r3_v1";
...
const L5T2_ABC = "ABCDEFGHIJLMNOPRSTUVY".split("");
function l5t2R3Build() {
  ...
  const tiles = l3Shuffle(letters.concat(decoys)).map((ch, i) => ({ id: i, ch }));
...
function PR3Palabra({ onSolve, verifyRef, eraseRef }) {
  ...
  const [verified, setVerified] = useStateG(false);
  const trayById = {}; b.tiles.forEach((t) => (trayById[t.id] = t));
  ...
  const assembled = slots.map((id) => (id === null ? "" : trayById[id].ch)).join("");
  function verificar() {
    if (verified) return;
    setVerified(true);
...
function AmazoniaGame({ app, setApp, go }) {
  ...
  function onSolve(isCorrect, entry) {
    ...
    setLog(newLog); setStars(newStars);
    if (isCorrect) setApp((s) => ({ ...s, stars: (s.stars || 0) + 1 }));
    const showFbAt = isCorrect ? 0 : (ROUNDS[round].verify ? 2300 : 1800);
    const advanceAt = showFbAt + (isCorrect ? 1150 : 1000);
    setTimeout(() => { setFeedback(isCorrect ? "ok" : "err"); setFeedbackMsg(isCorrect ? "+1 ⭐" : L3_ANIMOS[round % L3_ANIMOS.length]); }, showFbAt);
    setTimeout(() => {
      setFeedback(null); setFeedbackMsg("");
