...
function GameScreen({ app, setApp, go }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      ...
      {/* ── PAREJAS (indicador de progreso, alineado al patrón "Ronda"+dots del
          shell: top 20, ed-label 11, dots 11×11). Etiqueta "Parejas" porque en
          un memory la unidad de progreso es la pareja, no la ronda. ── */}
      <div style={{ position: "absolute", top: 20, left: "50%", transform: "translateX(-50%)", display: "flex", alignItems: "center", gap: 8 }}>
        <span className="ed-label" style={{ color: "rgba(255,255,255,0.7)", fontSize: 11, letterSpacing: "0.06em" }}>Parejas</span>
        {Array.from({ length: PAIRS }).map((_, i) => (
          <div key={i} style={{ width: 11, height: 11, borderRadius: "50%", background: i < matchedCount ? "#fce9a8" : "rgba(255,255,255,0.22)", boxShadow: i < matchedCount ? "0 0 8px #f2c260" : "none", transition: "all 0.2s" }} />
        ))}
      ...
      {/* right:196 deja un colchón de ~40px entre la 4ª carta y la columna de
          ...
          tocara los botones. */}
      <div style={{ position: "absolute", top: 64, left: 236, right: 236, bottom: 12, display: "flex", flexDirection: "column", justifyContent: "space-evenly", alignItems: "center" }}>
