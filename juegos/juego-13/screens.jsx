// screens.jsx — JUEGO-13 · "Un mundo por descubrir" (Estudios Sociales · 12 años).
// Juego MULTI-TEMA: el Home muestra N BOTONES = N temas del libro (patrón EDINUN).
// El niño elige un tema (chip) + su nombre → CharacterScreen (Domi preseleccionada) →
...
//   · continentes → TEMA 1 "Los continentes" (12 años, 3 rondas).
//   · americas → TEMA 2 "Las Américas y su geografía" (12 años, 3 rondas).
//   · diversidad → TEMA 3 "La diversidad cultural de la población mundial" (13 años,
//     3 rondas). Ya no hay temas en "Próximamente".
// Gradientes POR POSICIÓN (1º naranja · 2º amarillo · 3º azul), nunca por temática.
// ─────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────
// Salida del juego hacia la LISTA DE JUEGOS de EDINUN (botón "← JUEGOS" del Home).
//
// `JUEGOS_URL` es RELATIVA a la carpeta del juego: dos niveles arriba está el landing
// (juegos/<slug>/index.html → index.html de la raíz). Así funciona con doble clic, con un
// servidor local y si se sube el repo entero.
// ⚠ En producción los juegos se suben CARPETA POR CARPETA (ver SUBIR-A-SERVIDOR.md): si
// el landing no queda dos niveles arriba, poner aquí la URL absoluta de edinun.com.
//
// ⚠ El juego se incrusta en un <iframe> desde una página envoltorio: navegar `window.location`
// solo cambiaría el marco y el niño quedaría con la web de EDINUN metida dentro del juego.
// Por eso se navega la ventana de ARRIBA. Si el envoltorio fuera de otro dominio, el acceso
// a `window.top.location` lanza y se cae al comportamiento normal (try/catch).
// ─────────────────────────────────────────────────────────────
const JUEGOS_URL = "../../index.html";

function irAJuegos() {
  let destino = JUEGOS_URL;
  try { destino = new URL(JUEGOS_URL, window.location.href).href; } catch (e) {}
  try {
    if (window.top && window.top !== window.self) { window.top.location.href = destino; return; }
  } catch (e) {}
...
const LEVELS_CFG = [
  ...
  },
  {
    ...
    catLabel: "Las Américas y su geografía",
    enabled: true,
  },
  {
    id: "diversidad",
    ...
    catLabel: "La diversidad cultural",
    enabled: true,
  },
];

// ─────────────────────────────────────────────────────────────
// Fondo cósmico + glifos flotantes — geografía mundial (continentes, relieve,
// hidrografía y datos comparados).
// ─────────────────────────────────────────────────────────────
    ...
    return (
      ...
      >
        <div className="ed-glyphs" style={{ color: "rgba(255,255,255,0.10)", ...glyphsStyle }}>
          ...
          <span style={{ left: "82%", top: "16%", "--rot": "6deg", fontSize: "0.6em" }}>🗺️</span>
          ...
          <span style={{ left: "55%", top: "30%", "--rot": "-4deg", fontSize: "0.5em" }}>🏜️</span>
        </div>
      </div>
    );
  }
  return (
    <>
      ...
      <div className="ed-glyphs" style={glyphsStyle}>
        ...
        <span style={{ left: "70%", top: "24%", "--rot": "-6deg", fontSize: "0.58em" }}>⛰️</span>
...
// ─────────────────────────────────────────────────────────────
// 1. PANTALLA DE INICIO — 3 botones de tema + nombre + ENTRAR.
...
function HomeScreen({ app, setApp, go }) {
  ...
  const initial = app.level && LEVELS_CFG.some((l) => l.id === app.level && l.enabled) ? app.level : "continentes";
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      {/* Salida a la lista de juegos EDINUN (arriba izq.). Ver JUEGOS_URL / irAJuegos. */}
      <button className="ed-btn ed-btn-ghost" onClick={irAJuegos}
        style={{ position: "absolute", top: 18, left: 24, padding: "8px 14px", fontWeight: 800, letterSpacing: "0.04em", zIndex: 5 }}>
        ← JUEGOS
      </button>
...
// ─────────────────────────────────────────────────────────────
// 2. SELECCIÓN DE PERSONAJE — Domi preseleccionada (ciclo del elenco por ordinal
// del slug: 13 mod 4 = 1 → domi; ver memory/orden-personajes.md).
