...
const J13_T3R1PRO_KEY = "edinun_j13_t3r1pro_v1";
const J13_T3R1CON_KEY = "edinun_j13_t3r1con_v1";

// 5 cartas por ronda, SIEMPRE 3+2 o 2+3: nunca las 5 del mismo lado (se resolvería por
// inercia, sin leer). Cap de subconjunto: pro 3 de 14 → 8 · con 2 de 8 → 4 (cap < N−K).
function j13T3R1Build() {
  const nPro = Math.random() < 0.5 ? 3 : 2;
  const nCon = 5 - nPro;
  const pi = j13PickIdx(J13_DIV_PRO, nPro, J13_T3R1PRO_KEY);
  const ci = j13PickIdx(J13_DIV_CON, nCon, J13_T3R1CON_KEY);
  j13Commit(J13_T3R1PRO_KEY, pi, 8);
  j13Commit(J13_T3R1CON_KEY, ci, 4);
  const cartas = j13Shuffle(
    pi.map((i) => ({ t: J13_DIV_PRO[i].t, v: 1 })).concat(ci.map((i) => ({ t: J13_DIV_CON[i].t, v: -1 })))
  ).map((c, n) => Object.assign({}, c, { id: n }));
  return { cartas };
}
  ...
  function rotulo(v) {
    ...
    return (
      <button onClick={() => decidir(v)} disabled={!!fallo}
        style={{ flexShrink: 0, width: 96, minHeight: 76, borderRadius: 14, border: `2.5px solid ${on ? col : "rgba(255,255,255,0.28)"}`, background: on ? (v > 0 ? "rgba(46,204,143,0.28)" : "rgba(255,107,107,0.28)") : "rgba(0,0,0,0.3)", color: on ? "#fff" : "rgba(255,255,255,0.72)", fontFamily: "var(--ed-font-display)", fontWeight: 800, fontSize: 12.5, lineHeight: 1.2, padding: "8px 6px", cursor: fallo ? "default" : "pointer", boxShadow: on ? `0 0 18px ${col}` : "none", transition: "all 0.15s ease" }}>
        <div style={{ fontSize: 17, marginBottom: 2 }}>{v > 0 ? "▶" : "◀"}</div>
        {txt}
      </button>
    );
  }
        ...
        <div style={{ position: "relative", flex: 1, minWidth: 0, display: "flex", justifyContent: "center" }}>
          {carta && (
            <div onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp} onPointerCancel={onUp}
              ...
              {fallo && (
                <span style={{ position: "absolute", top: -12, right: -10, fontSize: 15, fontWeight: 900, color: "#fff", background: fallo.ok ? "#1f8a54" : "#c0392b", borderRadius: "50%", width: 30, height: 30, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 3px 8px rgba(0,0,0,0.4)" }}>{fallo.ok ? "✓" : "✗"}</span>
              )}
              {fallo && !fallo.ok && (
...
function T3R3Lluvia({ onSolve }) {
  ...
  useEffectG(() => {
    if (fin || ms < b.fin || cerrado.current) return;
    cerrado.current = true;
    setFin(true);
  ...
  function tocar(p) {
    if (fin || tomadas[p.id] || erradas[p.id]) return;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", height: "100%", width: "100%", paddingTop: 30 }}>
      ...
      <div style={{ position: "relative", width: "100%", height: PISTA, marginTop: 10, borderRadius: 14, border: "2px solid rgba(242,194,96,0.35)", background: "linear-gradient(180deg, rgba(10,6,35,0.35), rgba(10,6,35,0.05))", overflow: "hidden" }}>
        {b.palabras.map((p) => {
          const prog = (ms - p.t0) / J13_T3R3_CAIDA;
          const quieta = tomadas[p.id] || erradas[p.id];
          if (prog < 0 || (prog > 1 && !quieta)) return null;
          if (quieta && prog > 1.35) return null;
          const y = Math.min(prog, 1) * (PISTA - 40);
          const marcada = tomadas[p.id] ? "#2ecc8f" : erradas[p.id] ? "#ff6b6b" : null;
          return (
            <button key={p.id} onClick={() => tocar(p)} disabled={fin || !!quieta}
              style={{ position: "absolute", left: `${4 + p.carril * 32.6}%`, top: y, width: "30%", minHeight: 36, borderRadius: 11, border: `2.5px solid ${marcada || "#f2c260"}`, background: marcada ? (tomadas[p.id] ? "rgba(46,204,143,0.9)" : "rgba(255,107,107,0.9)") : "linear-gradient(180deg, rgba(255,255,255,0.97), rgba(240,235,225,0.93))", color: marcada ? "#fff" : "#2a2118", fontFamily: "var(--ed-font-display)", fontWeight: 800, fontSize: 11.5, lineHeight: 1.15, padding: "5px 6px", cursor: quieta || fin ? "default" : "pointer", boxShadow: "0 5px 14px rgba(0,0,0,0.35)", transform: quieta ? "scale(1.08)" : "none", transition: "transform 0.15s ease" }}>
              {p.txt}
            </button>
          );
        })}
...
function J13Game({ app, setApp, go, rounds }) {
  ...
  function onSolve(isCorrect, entry, gained) {
    ...
    setTimeout(() => { setFeedback(isCorrect ? "ok" : "err"); setFeedbackMsg(g > 0 ? `+${g} ⭐` : ANIMOS[round % ANIMOS.length]); }, showFbAt);
    setTimeout(() => {
