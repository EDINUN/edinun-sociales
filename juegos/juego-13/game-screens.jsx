...
function T3R1Muro({ onSolve }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", height: "100%", width: "100%", paddingTop: 58, overflow: "hidden" }}>
      ...
      {/* ⚠ `padding: 0 8` obligatorio: al acertar el riel se agranda a scale(1.05) y sin este
          margen su borde caía justo sobre el `overflow:hidden` de la zona — se veía cortado
          en recto, como si asomara otra tarjeta detrás. El resplandor exterior se cambió por
          uno INTERIOR por la misma razón: 26 px de brillo hacia afuera se cortaban en seco. */}
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, minHeight: 0, width: "100%", paddingLeft: 8, paddingRight: 8, boxSizing: "border-box" }}>
        {rotulo(-1)}
...
const J13_DIV_TABLEROS = [
  {
    ...
    parejas: [
      { k: "taegeukgi", e: "☯️", img: "assets/cultura-taegeukgi.png", fit: "contain", t:"Taegeukgi", r: "🇰🇷 Corea del Sur" },
      { k: "holi", e: "🎨", img: "assets/cultura-holi.png", t: "Festival de Holi", r: "🇮🇳 India" },
      { k: "padaung", e: "💍", img: "assets/cultura-padaung.png", t: "Pueblo Padaung", r: "🇹🇭 Tailandia" },
      { k: "intiraymi", e: "☀️", img: "assets/cultura-intiraymi.png", t: "Inti Raymi", r: "🇪🇨 Ecuador" },
    ],
  },
  {
    id: "signif", rel: "cada cultura con lo que representa",
    parejas: [
      { k: "taegeuk", e: "☯️", img: "assets/cultura-taegeukgi.png", fit: "contain", t:"Círculo Taegeuk", r: "El equilibrio entre fuerzas opuestas" },
      { k: "holi", e: "🎨", img: "assets/cultura-holi.png", t: "Festival de Holi", r: "La llegada de la primavera" },
      { k: "padaung", e: "💍", img: "assets/cultura-padaung.png", t: "Pueblo Padaung", r: "La belleza ligada al largo del cuello" },
      { k: "intiraymi", e: "☀️", img: "assets/cultura-intiraymi.png", t: "Inti Raymi", r: "El agradecimiento al Sol por la cosecha" },
    ],
  },
];
...
function j13T3R2Build() {
  ...
  const cartas = [];
  parejas.forEach((p, n) => {
    cartas.push({ id: "a" + n, par: n, cara: "a", e: p.e, img: p.img, fit: p.fit, t: p.t });
      ...
      <button key={c.id} onClick={() => tocar(c)} disabled={revelado}
        ...
        <div style={{ position: "relative", width: "100%", height: "100%", transformStyle: "preserve-3d", transition: "transform 0.36s ease", transform: arriba ? "rotateY(180deg)" : "none" }}>
          ...
          <div style={{ position: "absolute", inset: 0, backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", transform: "rotateY(180deg)", borderRadius: 13, border: `2.5px solid ${borde}`, background: "linear-gradient(180deg, rgba(255,255,255,0.97), rgba(240,235,225,0.93))", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4, padding: "6px 6px", boxShadow: resuelta ? "0 0 14px rgba(46,204,143,0.6)" : "0 6px 16px rgba(0,0,0,0.35)" }}>
            {c.img ? (
              /* La ilustración ocupa la CARTA ENTERA y el nombre va encima, sobre un velo
                 oscuro. En una franja de 62 px arriba, un motivo vertical (la mujer
                 Padaung) salía a 54×60 px con medio cuadro de fondo vacío a los lados:
                 la autora lo reportó dos veces como "no se ve nada". A sangre sale a
                 ~98×118, el triple de superficie.
                 · Las ilustraciones van `cover` (llenan y recortan por los lados, el motivo
                   está centrado); la bandera va `contain` sobre blanco porque recortarla
                   le comería los trigramas de las esquinas.
                 · El `overflow:hidden` vive en ESTA capa, no en la cara de la carta: los
                   distintivos ✓ y ●▲■◆ van pegados por fuera del borde y se recortarían. */
              <div style={{ position: "absolute", inset: 0, borderRadius: 11, overflow: "hidden", display: "flex", alignItems: "flex-end" }}>
                {/* `contain` se sube al 10 %: centrada, la bandera quedaba con los dos
                    trigramas de abajo detrás de la banda del nombre. */}
                <img src={c.img} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: c.fit || "cover", objectPosition: c.fit === "contain" ? "center 10%" : "center", background: "#fff" }} />
                {/* Velo OPACO, no un degradado suave: el nombre cae sobre la parte baja de
                    la ilustración, que puede ser clarísima (la túnica blanca del Padaung,
                    las frutas del Inti Raymi) y en blanco sobre blanco no se leía. El
                    `paddingBottom` grande deja libre el pie de la carta, donde asoma el
                    distintivo ●▲■◆ de la pareja al destapar el tablero. */}
                <span style={{ position: "relative", width: "100%", padding: "18px 5px 13px", background: "linear-gradient(180deg, rgba(10,6,26,0) 0%, rgba(10,6,26,0.88) 48%, rgba(10,6,26,0.95) 100%)", fontFamily: "var(--ed-font-display)", fontWeight: 800, fontSize: c.t.length > 26 ? 9.5 : 11, lineHeight: 1.15, color: "#fff", textAlign: "center", textShadow: "0 1px 3px rgba(0,0,0,0.9)" }}>{c.t}</span>
              </div>
            ) : (
              <React.Fragment>
                {c.e && <span style={{ fontSize: 26, lineHeight: 1 }}>{c.e}</span>}
...
function T3R3Lluvia({ onSolve }) {
  ...
  return (
    /* Única ronda con el enunciado a 46 y no a 58: la pista de caída ocupa casi toda la
       zona y a 58 el enunciado quedaba pegado al borde de la caja (pedido de la autora). */
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", height: "100%", width: "100%", paddingTop: 46 }}>
      <div style={{ pointerEvents: "none", textAlign: "center", textShadow: "0 2px 8px rgba(0,0,0,0.6)" }}>
