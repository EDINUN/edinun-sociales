        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 22, maxWidth: 520 }}>
          <div>
            <div className="ed-label" style={{ color: "#4fd8ff", marginBottom: 8 }}>
              EDINUN · Reconozco el camino para llegar a mi casa {/* ← PERSONALIZAR: tema del juego */}
            </div>
          ...
          <div style={{
            ...
            Encuentra el camino a casa jugando. {/* ← PERSONALIZAR: subtítulo corto */}
          </div>
...
function CharacterScreen({ app, setApp, go }) {
  const [sel, setSel] = useState("yaku"); // ← Yaku SIEMPRE preseleccionado (orden Domi→Yaku→Sisa→Andi)
  ...
  function choose() {
    setApp((s) => ({
      ...s,
      character: sel,
      currentCategory: "camino",
      currentCatLabel: "El camino a casa",
    }));
    go("game");
  }
